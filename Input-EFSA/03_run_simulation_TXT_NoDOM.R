...
df_dominat <- readRDS("Input-EFSA/df_dominat_control.rds")
df_dominat <- df_dominat[df_dominat$PFT_rank < 21 & df_dominat$group_I1 == group, ]
df_fieldedge <- df_fieldedge_INIT[!(df_fieldedge_INIT$Species %in% df_dominat$PFT), ]
#  -------------------------------------------------
...
while(MC<6 & IT<21){
  ...
  # Construct and run the command
    ...
    # 60s 
    mycall <- paste('timeout 60 ./IBCgrassGUI', ModelVersion, CellNum, Tmax, InitDuration,
